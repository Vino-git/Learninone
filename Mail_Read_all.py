...
if __name__ == '__main__':
  ...
  bins = raw_input("Enter the Bin with comma separated")
  try:
    for i in range (session.Folders.Count):
      ...
      for folder, folders, items in cdo_walk (info_store):
        ...
        for item in folder.Items:
          for jbin in bins:
            print "i am inside"
            if re.search(jbin+'[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]',item.Body):
              ful_card = re.search(jbin+'[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]',item.Body)
              check_digit = card_check_digit(ful_card.group(0))
              if check_digit:
                print item.Subject
          break
